...
void SparseMatrix::InitOutLine(SparseMatrixOutline &outline)const{
  
  outline.SetRows(n_);
  for (int r = 0; r < n_; ++r){
    for (int c_i = 0; c_i < rowLength_[r]; ++c_i){
	  const int c = cindices_[r][c_i];
	  const double value = centries_[r][c_i];
	  outline.AddEntry(r,c,value);
	}
  }
}
...
{
  //mat2 and this matrix should have the same structure
  assert( mat2.Getn() == this->Getn());//added by lsw
...
void SparseMatrix::RemoveColumns(const vector<int> &columns, int oneIndexed){

  if(columns.size() > 0){
	const int numColumns = (int)columns.size();
	const int *pcolumns = &columns[0];
	RemoveColumns(numColumns,const_cast<int*>(pcolumns),oneIndexed);
  }
}
...
void SparseMatrix::RemoveRowsColumns(const vector<int> &removedRowsColumns, int oneIndexed){

  if(removedRowsColumns.size() > 0){
	
	const int numRemovedRowsColumns = (int)removedRowsColumns.size();
	const int *premovedRowsColumns = &removedRowsColumns[0];
	RemoveRowsColumns(numRemovedRowsColumns,const_cast<int*>(premovedRowsColumns),oneIndexed);
  }
}
...
{
  memset(denseMatrix, 0, GetNumRows() * GetNumColumns()* sizeof(double));
  for(int i=0; i< n_; i++)
    for(int j=0; j<rowLength_[i]; j++)
