...
/// the namesape is added by lsw.
namespace BARBIC_LIB{
  ...
  protected:
	...
	/**
	 * entries[i] is the i-th row of the matrix, and each pair in entries[i] is
	 * (colunm_index,data_value).
	 * 
	 */
  ...
  public:
	...
	// added by lsw, in order to convert this type of sparse matrix into other type conveniently.
	void InitOutLine(SparseMatrixOutline &sparseMatrixOutline)const;
	...
	// warning: this function makes a local copy; "ScalarMultiply" is more efficient
	friend SparseMatrix operator* (const double alpha, const SparseMatrix & mat2){
	...
	void RemoveRowsColumns(const std::vector<int> &rowColumns, int oneIndexed=0); // the rowColumns must be sorted (ascending), added by lsw.
	...
	void RemoveColumns(const std::vector<int>& columns, int oneIndexed=0); // 0-indexed; columns must be sorted (ascending), added by lsw
	...
	inline int * GetRowLenHandle() const { return rowLength_; }
	inline int ** GetColIndHandle() const { return cindices_; }

  protected:

	// compact representation for fast matrix multiplications
	int n_; //number of rows
	int * rowLength_; // rowLength_[i] is the number of the nonzeros of i-th row, e.g. the length of cindices_[i] and centries_[i].
	int ** cindices_; // cindices_[i] is the column index of the nonzeros of i-th row.
	double ** centries_;// centries_[i][j] is the value of the elements with index of (i,cindices_[i][j]).
