...
void VolMeshDrawer::render(pTetMesh_const volmesh,VOLMESHRENDERTYPE t,const double *u){
...
void VolMeshDrawer::drawPoint(pTetMesh_const volmesh){

  if(volmesh == NULL){
	WARN_LOG("the TetMesh is null");
	return;
  }
  ...
  glColor3d(1, 1, 0);
  glBegin(GL_POINTS);
  for (int i=0; i<volmesh->nodes().size(); i++){
	const Vector3d &v = volmesh->nodes()[i];
...
void VolMeshDrawer::drawPoint(pTetMesh_const volmesh,const double *u){
...
void VolMeshDrawer::drawEdge(pTetMesh_const volmesh){
  
  ///@todo draw edges of volumetric mesh.
  
}

void VolMeshDrawer::drawEdge(pTetMesh_const volmesh,const double *u){
...
void VolMeshDrawer::drawElement(pTetMesh_const volmesh,const double *u){
...
void VolMeshDrawer::drawElement(pTetMesh_const volmesh){
