...
void insert_I3x3(SparseMatrixXd &m,int node_id){
...
const SparseMatrixXd &FullBaryCenConM::computeConM(const set<int> &con_nodes,int total_node_num,SparseMatrixXd &con_m){
...
const SparseMatrixXd &FullBaryCenConM::computeConM(const vector<set<int> > &con_nodes,int total_node_num,SparseMatrixXd &con_m){

  int nz = 0;
  VecT C_triplet;
  computeConM(con_nodes,total_node_num,C_triplet);
...
void FullBaryCenConM::computeConM(const vector<set<int> >&con_nodes,int total_node_num){

  con_Ms.resize(con_nodes.size());
  vector<set<int> >::const_iterator it = con_nodes.begin();
  for( int i = 0; it != con_nodes.end(); it ++ ,i++ ){
	FullBaryCenConM::computeConM(*it,total_node_num,con_Ms[i]);
  }
}

const VecT &FullBaryCenConM::computeConM(const vector<set<int> > &con_nodes, 
										 int total_node_num,
										 VecT &C_triplet){
  ...
  for( int i = 0; node_it != con_nodes.end(); node_it ++ ,i++ ){
	...
	SparseMatrixXd C;
	FullBaryCenConM::computeConM(*node_it,total_node_num,C);
	for (int k=0; k<C.outerSize(); ++k){
	  for (SparseMatrixXd::InnerIterator it(C,k); it; ++it){
