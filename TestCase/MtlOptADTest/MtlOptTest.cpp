...
const int max_outter_it = 10;
const double outer_tol = 1e-4;
const int save_per_it = 25;
...
public:
  MtlOptADTestSuite(const string test_case_n):test_case_name(test_case_n){
	...
	set<int> fixed_nodes;
	BOOST_FOREACH(const int ele, model->fixed_nodes){
	  fixed_nodes.insert(ele);
	}
  ...
  void save(){
	...
	TEST_ASSERT( EIGEN3EXT::write(f+"OptS.b",dataM->S) );
  }
  void printZ(){
  ...
  void saveMesh(){
	...
	const string dir = inf_path.parent_path().string();
	const string fname = inf_path.filename().string();
	const string mesh_dir_f = dir+"/tempt/mesh/"+fname+test_case_name;
	model->saveMesh(dataM->getSZ(),mesh_dir_f+"output_mesh_");
  }
...
BOOST_AUTO_TEST_CASE(Opt_Z_AtA_updateW){

  MtlOptADTestSuite mtlOpt("Opt_Z_AtA_updateW");
  mtlOpt.addOptimizer(pMtlOptimizer(new ZOptimizer(mtlOpt.getDataModel())));
  mtlOpt.addOptimizer(pMtlOptimizer(new AtAOptimizer(mtlOpt.getDataModel())));
  const int rW = mtlOpt.model->W.cols();
  mtlOpt.addOptimizer(pMtlOptimizer(new WOptimizer(mtlOpt.getDataModel(),rW)));
  mtlOpt.solve();
}

BOOST_AUTO_TEST_CASE(Opt_Z_Lambda_updateW){

  MtlOptADTestSuite mtlOpt("Opt_Z_Lambda_updateW");
  mtlOpt.addOptimizer(pMtlOptimizer(new ZOptimizer(mtlOpt.getDataModel())));
  mtlOpt.addOptimizer(pMtlOptimizer(new LambdaOptimizer(mtlOpt.getDataModel())));
  const int rW = mtlOpt.model->W.cols();
  mtlOpt.addOptimizer(pMtlOptimizer(new WOptimizer(mtlOpt.getDataModel(),rW)));
